...
import type { ImageAsset, OfferContent, Palette } from "@/lib/offers/types";
...
/**
 * Converte a paleta da oferta em custom properties CSS. Sobrescreve, só nesta
 * rota, os tokens globais do Tailwind (`--color-*`) e os do CSS `scrap-*`
 * (`--p-*`, `--cta`) — assim a home e as demais páginas mantêm suas cores.
 */
function paletteVars(p: Palette): React.CSSProperties {
  return {
    ...
    "--p-marigold": p.marigold,
    "--p-card": p.card,
  } as React.CSSProperties;
}

/** Renderiza uma string com "\n" como múltiplas linhas (com <br/>). */
...
function SecurePurchaseBadge({ badge }: { badge: ImageAsset }) {
  return (
    <LazyImg
      src={badge.src}
      alt={badge.alt}
        ...
        <div className="mx-auto flex w-full max-w-[480px] flex-col items-center gap-5">
          ...
          <div role="heading" aria-level={1} className="leading-[1.05]">
            <p className="text-[22.5px] font-normal sm:text-[26px]">
              <span
                style={{
                  fontFamily: HH.highlight.font ?? "var(--font-agbalumo)",
                  fontSize: HH.highlight.scale ? `${HH.highlight.scale}em` : undefined,
                  fontWeight: HH.highlight.weight,
                  color: HH.highlight.color,
                  backgroundColor: HH.highlight.bg,
              ...
              >
                <Multiline text={hero.titleHighlight} />
              </span>
            </p>
            {hero.title ? (
              <p className="text-[22px] font-semibold" style={{ marginTop: "0.35em" }}>
                <span
                  style={{
                    ...
                    fontSize: HH.title.scale ? `${HH.title.scale}em` : undefined,
                    color: HH.title.color,
                    backgroundColor: HH.title.bg,
                    padding: "0.15em 0.5em",
                    borderRadius: "0.6em",
                    WebkitBoxDecorationBreak: "clone",
                    boxDecorationBreak: "clone",
        ...
        <div className="mx-auto flex w-full max-w-[553px] flex-col items-center gap-7 lg:max-w-[960px]">
          ...
          {testimonials.marquee ? (
            <Marquee
              ...
              itemWidth={testimonials.marquee.itemWidth ?? 260}
              containerClassName={testimonials.marquee.containerClassName ?? "w-full"}
              imageSize={
                testimonials.marquee.imageSize ?? {
                  width: testimonials.items[0]?.width ?? 819,
                  height: testimonials.items[0]?.height ?? 1024,
                }
              }
              items={testimonials.items.map((it) => ({
                src: it.src,
                alt: it.alt,
